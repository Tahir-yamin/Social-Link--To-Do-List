...
// Caches for API responses to prevent duplicate calls
const metadataCache = new Map<string, Promise<LinkMetadata>>();
const analysisCache = new Map<string, Promise<string>>();
...
export const fetchDeepAnalysis = async (url: string): Promise<string> => {
    ...
    analysisCache.set(url, fetchPromise);
    return fetchPromise;
};