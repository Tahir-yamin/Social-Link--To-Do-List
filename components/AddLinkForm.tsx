...
const AddLinkForm: React.FC<AddLinkFormProps> = ({ onAddLink }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Bypass the AI service and create the link directly
    const newLink = {
      url: url.trim(),
      title: url.trim(), // Use the URL as the title
      summary: "AI-powered summaries are available when running locally with an API key.",
      category: "General",
      sources: [],
    ...
    <div className="p-4 bg-white dark:bg-slate-800 rounded-lg shadow-lg">
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        <div className="flex flex-col sm:flex-row items-center gap-3">
            ...
            placeholder="Paste a link to save it to your list... (Ctrl+K)"
            ...
            />
             <div className="flex items-center gap-2 w-full sm:w-auto">
                ...
                <button
                type="submit"
                className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-indigo-600 text-white font-semibold rounded-md shadow-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-slate-800 transition-colors"
                >
